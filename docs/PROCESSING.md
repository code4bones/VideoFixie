...
Video2X quality should be compared on identical source content before choosing a final profile. The Variants workflow builds several temporary Video2X profiles for the current TestSegment, prepares the shared preview source once, and runs Video2X variants through a bounded queue with the same preview output preset. The UI exposes a parallelism limit capped at 6 active Video2X variants; use lower values if VRAM pressure appears.
