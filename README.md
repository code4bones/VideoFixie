...
Use `Variants` to run a sequential Video2X benchmark grid over the current cut and compare practical RealCUGAN/RealESRGAN live-action variants without manually changing profile settings between runs.
...
- `Save Cut` asks for a cut name, then stores the segment, selected backend/profile and output preset for the source file;
- a source can have multiple saved cuts, shown in the `Cut` list;
- `Load Cut` opens a choice dialog and restores the selected segment and its profile/output choices;
...
If `./bin/Video2X-x86_64.AppImage` is available, bootstrap an inspectable local runtime with:
